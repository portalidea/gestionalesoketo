...
export default function InvitedRetailerOrder() {
  ...
  return <main className="min-h-screen bg-[#FAF7F0] text-[#254521]">
    ...
    <div className="mx-auto grid max-w-7xl gap-7 px-5 py-8 lg:grid-cols-[minmax(0,1fr)_390px]">
      <section>
        ...
        <div className="mt-5 overflow-hidden rounded-xl border border-[#2D5A27]/10 bg-white">
          {data.products.map((product) => <div className="grid gap-3 border-b border-[#2D5A27]/10 px-4 py-4 last:border-0 md:grid-cols-[minmax(0,1fr)_120px_112px] md:items-center" key={product.id}>
            ...
            <p className="w-[120px] whitespace-nowrap font-semibold text-[#2D5A27]">{euro(product.unitListNet)} <span className="text-xs font-normal text-[#6B7C66]">IVA escl.</span></p>
            <Input aria-label={`Quantità ${product.name}`} className="h-11 w-[112px] border-[#2D5A27]/40 bg-[#F3F7ED] text-center font-semibold text-[#254521] shadow-sm focus-visible:ring-[#7AB648]" type="number" min="0" step="1" placeholder="0" value={quantities[product.id] ?? ""} onChange={(event) => setQuantities((previous) => ({ ...previous, [product.id]: event.target.value }))} />
          </div>)}
        </div>

        {result && <section className="mt-7">
          <div className="mb-4"><h2 className="font-[Poppins] text-xl font-bold text-[#2D5A27]">Prezzi del tuo assortimento</h2><p className="mt-1 text-sm text-[#52634d]">Listino e fasce sono prezzi unitari per rivenditori, <strong>IVA esclusa</strong>.</p></div>
          <div className="overflow-x-auto rounded-xl border border-[#2D5A27]/10 bg-white shadow-sm lg:overflow-visible">
          ...
          <p className="mt-3 rounded-lg border border-[#2D5A27]/15 bg-[#F3F7ED] px-3 py-2.5 text-xs leading-5 text-[#426b2f]"><strong>Prezzi IVA esclusa.</strong> Il listino indica il prezzo consigliato di vendita al pubblico al netto dell’IVA; le quattro colonne mostrano il prezzo unitario riservato a ciascuna fascia.</p>
        </section>}
      </section>

      <aside className="space-y-5 lg:sticky lg:top-5 lg:self-start">
        <Card className="overflow-hidden border-[#2D5A27]/15"><CardHeader className="bg-[#2D5A27] text-white"><CardTitle className="text-white">Riepilogo ordine</CardTitle><CardDescription className="text-[#EAF3E6]">Dati calcolati sul server.</CardDescription></CardHeader><CardContent className="space-y-3 bg-[#FAF7F0] p-5">{calculation.isFetching ? <Loader2 className="h-5 w-5 animate-spin text-[#2D5A27]" /> : result ? <><p className="text-sm text-[#426b2f]">Totale listino netto <strong className="float-right text-[#254521]">{euro(result.listSubtotalNet)}</strong></p><div className="rounded-lg bg-[#F3F7ED] p-3"><p className="text-xs font-semibold uppercase text-[#426b2f]">Fascia raggiunta</p><p className="text-xl font-bold text-[#2D5A27]">{result.reachedTier.name}</p><p className="text-sm text-[#426b2f]">Prezzo merce netto: <strong className="text-[#254521]">{euro(result.currentTierMerchandiseNet)}</strong></p>{result.nextTier && <p className="mt-2 text-sm text-[#426b2f]">Mancano {euro(result.nextTier.additionalListNet)} di listino per {result.nextTier.name}.</p>}</div><p className="flex gap-2 text-sm text-[#426b2f]"><Truck className="h-4 w-4 text-[#5D973B]" />{result.freeShippingApplied ? "Spedizione gratuita" : `Spedizione ${euro(result.shippingNet)} IVA esclusa`}</p><p className="flex gap-2 text-sm text-[#426b2f]"><Package className="h-4 w-4 text-[#5D973B]" />{result.displayStandUnlocked ? "Espositore in omaggio incluso" : `Espositore sopra ${euro(result.displayStandThreshold)} di listino netto`}</p></> : <p className="text-sm text-[#52634d]">Aggiungi prodotti per vedere il riepilogo.</p>}</CardContent></Card>
        <Card><CardHeader><CardTitle className="text-[#2D5A27]">Dati per la consegna</CardTitle><CardDescription>I dati dell’invito sono già compilati; completa quelli mancanti.</CardDescription></CardHeader><CardContent>{submitted ? <div className="rounded-lg bg-[#2D5A27] p-4 text-sm text-[#FAF7F0]"><CheckCircle2 className="mb-2 text-[#A9D57C]" /><strong className="text-white">Ordine ricevuto.</strong><p className="mt-1 text-[#EAF3E6]">Ti contatteremo per l’approvazione e l’evasione.</p></div> : <form className="space-y-3" onSubmit={(event) => { event.preventDefault(); if (complete) submit.mutate({ token, ...contact, privacyAccepted: true, items: cartItems }); }}><div className="hidden"><Input tabIndex={-1} value={contact.website} onChange={(event) => update("website", event.target.value)} /></div><Input required placeholder="Ragione sociale" value={contact.legalName} onChange={(event) => update("legalName", event.target.value)} /><Input required placeholder="Referente" value={contact.contactName} onChange={(event) => update("contactName", event.target.value)} /><Input required type="email" placeholder="Email" value={contact.email} onChange={(event) => update("email", event.target.value)} /><Input required placeholder="Telefono" value={contact.phone} onChange={(event) => update("phone", event.target.value)} /><Input required placeholder="Tipo attività" value={contact.businessType} onChange={(event) => update("businessType", event.target.value)} /><Input required placeholder="Via e numero civico" value={contact.address} onChange={(event) => update("address", event.target.value)} /><div className="grid grid-cols-2 gap-3"><Input required placeholder="CAP" value={contact.postalCode} onChange={(event) => update("postalCode", event.target.value)} /><Input required placeholder="Città" value={contact.city} onChange={(event) => update("city", event.target.value)} /></div><div className="grid grid-cols-2 gap-3"><Input required placeholder="Provincia (es. MI)" maxLength={2} value={contact.province} onChange={(event) => update("province", event.target.value.toUpperCase())} /><Input required placeholder="P. IVA" value={contact.vatNumber} onChange={(event) => update("vatNumber", event.target.value)} /></div><textarea className="min-h-20 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm" placeholder="Note per l’ordine (opzionali)" value={contact.notes} onChange={(event) => update("notes", event.target.value)} /><label className="flex gap-2 text-xs leading-5 text-[#52634d]"><Checkbox checked={contact.privacyAccepted} onCheckedChange={(value) => update("privacyAccepted", value === true)} /><span>Ho letto l’<a className="font-semibold text-[#2D5A27] underline" href={data.config.privacyPolicyUrl} target="_blank" rel="noreferrer">informativa privacy</a> e acconsento al trattamento dei dati per la gestione dell’ordine.</span></label>{submit.error && <p className="text-sm text-destructive">{submit.error.message}</p>}<Button className="w-full bg-[#2D5A27] text-[#FAF7F0] hover:bg-[#254521] hover:text-white" disabled={!complete || submit.isPending}>{submit.isPending ? <Loader2 className="animate-spin" /> : "Invia ordine"}</Button></form>}</CardContent></Card>
      </aside>
    </div>
