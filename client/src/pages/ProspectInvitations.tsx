import { Activity, Check, Copy, Link2, Loader2, Mail, RefreshCw, Send, ShieldOff } from "lucide-react";
...
type InvitationRow = {
  ...
  hasComposedWithoutSubmitting: boolean;
  lastCompositionAt: Date | string | null;
};
...
export default function ProspectInvitations() {
  ...
  const compositionDetail = trpc.prospectSimulator.adminInvitationCompositionDetail.useQuery(
    { id: selectedInvitationId ?? "00000000-0000-0000-0000-000000000000" },
  ...
  return <DashboardLayout><div className="mx-auto max-w-7xl space-y-6">
    ...
    <Card><CardHeader><CardTitle>Inviti creati</CardTitle><CardDescription>Un fallimento email non impedisce la copia del collegamento personale.</CardDescription></CardHeader><CardContent className="p-0"><div className="overflow-x-auto"><table className="w-full min-w-[1260px] text-sm"><thead className="border-y bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground"><tr><th className="px-5 py-3">Prospect</th><th className="px-4 py-3">Stato</th><th className="px-4 py-3">Notifica</th><th className="px-4 py-3">Creato</th><th className="px-4 py-3">Ultima apertura</th><th className="px-4 py-3">Composizione</th><th className="px-4 py-3">Richiesta</th><th className="px-4 py-3">Azioni</th></tr></thead><tbody>{invitations.isLoading ? <tr><td colSpan={8} className="px-5 py-10 text-center text-muted-foreground">Caricamento inviti…</td></tr> : invitationRows.length === 0 ? <tr><td colSpan={8} className="px-5 py-10 text-center text-muted-foreground">Nessun invito creato.</td></tr> : invitationRows.map((invite) => <tr key={invite.id} className="border-b last:border-0"><td className="px-5 py-4"><strong>{invite.legalName}</strong><span className="mt-0.5 block text-xs text-muted-foreground">{invite.contactName} · {invite.email}<br />{invite.phone}</span></td><td className="px-4 py-4"><Badge variant={statusVariant(invite.status)}>{invite.status}</Badge><span className="mt-1 block text-xs text-muted-foreground">Scade {date(invite.tokenExpiresAt)}</span></td><td className="px-4 py-4"><Badge variant={invite.notificationStatus === "sent" ? "default" : invite.notificationStatus === "failed" ? "destructive" : "secondary"}>{invite.notificationStatus}</Badge>{invite.notificationError && <span className="mt-1 block max-w-44 truncate text-xs text-destructive" title={invite.notificationError}>{invite.notificationError}</span>}</td><td className="px-4 py-4 text-muted-foreground">{date(invite.createdAt)}</td><td className="px-4 py-4 text-muted-foreground">{date(invite.lastOpenedAt)}</td><td className="px-4 py-4">{invite.hasComposedWithoutSubmitting ? <><Badge className="bg-amber-100 text-amber-950 hover:bg-amber-100">Da richiamare</Badge><span className="mt-1 block text-xs text-muted-foreground">Ultima attività {date(invite.lastCompositionAt)}</span></> : invite.lastCompositionAt ? <span className="text-xs text-muted-foreground">Nessun carrello composto</span> : "—"}</td><td className="px-4 py-4">{invite.simulationId ? <Badge variant="outline">Ricevuta</Badge> : "—"}</td><td className="px-4 py-4"><div className="flex gap-1"><Button variant="outline" size="icon" title="Vedi attività carrello" onClick={() => setSelectedInvitationId(invite.id)}><Activity className="h-4 w-4" /></Button><Button variant="outline" size="icon" title="Copia link" onClick={() => copyLink(invite.id, invite.token)}>{copiedId === invite.id ? <Check className="h-4 w-4 text-green-600" /> : <Copy className="h-4 w-4" />}</Button><Button variant="outline" size="icon" title="Reinvia email" disabled={invite.status === "revoked" || invite.status === "submitted" || resend.isPending} onClick={() => resend.mutate({ id: invite.id, origin: origin() })}><Mail className="h-4 w-4" /></Button><Button variant="outline" size="icon" title="Rigenera link" disabled={invite.status === "submitted" || regenerate.isPending} onClick={() => regenerate.mutate({ id: invite.id, origin: origin() })}><RefreshCw className="h-4 w-4" /></Button><Button variant="outline" size="icon" title="Revoca invito" disabled={invite.status === "revoked" || invite.status === "submitted" || revoke.isPending} onClick={() => revoke.mutate({ id: invite.id })}><ShieldOff className="h-4 w-4" /></Button></div></td></tr>)}</tbody></table></div><div className="border-t px-5 py-3 text-xs text-muted-foreground"><Link2 className="mr-1 inline h-3.5 w-3.5" />Copia link usa l’origine corrente del browser, senza dominio hard-coded.</div></CardContent></Card>
    <Dialog open={Boolean(selectedInvitationId)} onOpenChange={(open) => !open && setSelectedInvitationId(null)}><DialogContent className="max-h-[88vh] max-w-[calc(100%-2rem)] overflow-y-auto sm:max-w-[900px]"><DialogHeader><DialogTitle>Storico composizioni</DialogTitle><DialogDescription>Attività del carrello registrata per l’invito selezionato.</DialogDescription></DialogHeader>{compositionDetail.isLoading ? <p className="py-8 text-center text-muted-foreground">Caricamento attività…</p> : compositionDetail.data ? <div className="space-y-4"><div className="rounded-lg bg-muted/40 p-3 text-sm"><strong>{compositionDetail.data.invitation.legalName}</strong><span className="ml-2 text-muted-foreground">{compositionDetail.data.invitation.contactName}</span></div>{compositionDetail.data.sessions.length === 0 ? <p className="py-8 text-center text-sm text-muted-foreground">Nessuna composizione registrata.</p> : compositionDetail.data.sessions.map((session) => <details key={session.id} className="group rounded-lg border"><summary className="flex cursor-pointer list-none flex-wrap items-center justify-between gap-3 p-4"><div><p className="font-semibold">Ultima attività: {date(session.lastActivityAt)}</p><p className="mt-1 text-sm text-muted-foreground">Avviata {date(session.startedAt)} · {session.submitted ? "Sfociata nell’ordine inviato" : "Non inviata"}</p></div><div className="flex flex-wrap gap-2"><Badge variant="secondary">{session.reachedTier ?? "Carrello vuoto"}</Badge><Badge variant="outline">Listino {euro(session.listTotal)}</Badge><Badge variant="outline">Netto {euro(session.discountedNet)}</Badge>{session.nextTierDistanceEur != null && <Badge variant="outline">Mancano {euro(session.nextTierDistanceEur)}</Badge>}</div></summary><div className="border-t px-4 pb-4 pt-3"><p className="mb-2 text-sm font-medium">Carrello</p>{session.cartItems.length === 0 ? <p className="text-sm text-muted-foreground">Nessun prodotto selezionato.</p> : <div className="overflow-hidden rounded-md border"><table className="w-full text-sm"><thead className="bg-muted/40 text-left text-xs uppercase text-muted-foreground"><tr><th className="px-3 py-2">Prodotto</th><th className="px-3 py-2 text-right">Quantità</th></tr></thead><tbody>{session.cartItems.map((item) => <tr key={item.productId} className="border-t"><td className="px-3 py-2">{item.product ? <><strong>{item.product.name}</strong><span className="ml-2 text-xs text-muted-foreground">{item.product.sku}</span></> : <span className="text-muted-foreground">Prodotto non più disponibile ({item.productId})</span>}</td><td className="px-3 py-2 text-right font-semibold">{item.quantity}</td></tr>)}</tbody></table></div>}</div></details>)}</div> : compositionDetail.error ? <p className="text-sm text-destructive">{compositionDetail.error.message}</p> : null}</DialogContent></Dialog>
  </div></DashboardLayout>;
}
