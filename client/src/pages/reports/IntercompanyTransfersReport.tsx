...
export default function IntercompanyTransfersReport() {
  ...
  return <DashboardLayout><main className="container py-6 space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h1 className="text-2xl font-semibold">Travasi inter-company</h1><p className="text-sm text-muted-foreground">Solo trasferimenti SoKeto → E-Keto; nessuna fatturazione automatica.</p></div><Button onClick={() => exportCsv.mutate({ dateFrom, dateTo })} disabled={exportCsv.isPending}><Download className="mr-2 h-4 w-4" />Esporta CSV</Button></div>
    ...
    <Card><CardHeader><CardTitle className="flex items-center gap-2"><ArrowRightLeft className="h-5 w-5" />Riepilogo mensile · costo € {Number(report.data?.totalCost ?? 0).toFixed(2)}</CardTitle></CardHeader><CardContent>
      <Table><TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Direzione</TableHead><TableHead>Prodotto</TableHead><TableHead>Lotto</TableHead><TableHead>Q.tà</TableHead><TableHead>Costo</TableHead><TableHead>Ordine</TableHead><TableHead>Stato attuale</TableHead><TableHead>Operatore</TableHead></TableRow></TableHeader><TableBody>
        ...
        {!report.isLoading && !report.data?.items.length && <TableRow><TableCell colSpan={9} className="py-10 text-center text-muted-foreground">Nessun travaso nel periodo selezionato.</TableCell></TableRow>}
      </TableBody></Table>
    </CardContent></Card>
