...
export type ProspectNotificationPayload = {
  ...
  reachedTierDiscountPercent: string;
  itemCount: number;
};
...
export function buildProspectOrderNotification(payload: ProspectNotificationPayload): { subject: string; html: string } {
  const subject = `Nuovo ordine prospect — ${payload.legalName}`;
  const discountPercent = Number(payload.reachedTierDiscountPercent).toLocaleString("it-IT", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  const html = `<!doctype html><html lang="it"><body style="margin:0;background:#FAF7F0;color:#2D5A27;font-family:Poppins,Arial,sans-serif"><main style="max-width:640px;margin:24px auto;background:#fff;border-radius:12px;padding:32px"><h1 style="margin:0 0 18px;font-size:22px">Nuovo ordine</h1><p style="margin:0 0 22px">È stato salvato un nuovo ordine.</p><table role="presentation" cellspacing="0" cellpadding="8" style="border-collapse:collapse;width:100%"><tr><td><strong>Ragione sociale</strong></td><td>${escapeHtml(payload.legalName)}</td></tr><tr><td><strong>Referente</strong></td><td>${escapeHtml(payload.contactName)}</td></tr><tr><td><strong>Email</strong></td><td>${escapeHtml(payload.email)}</td></tr><tr><td><strong>Telefono</strong></td><td>${escapeHtml(payload.phone)}</td></tr><tr><td><strong>Attività</strong></td><td>${escapeHtml(payload.businessType)}</td></tr><tr><td><strong>Città</strong></td><td>${escapeHtml(payload.city)}</td></tr><tr><td><strong>P. IVA</strong></td><td>${escapeHtml(payload.vatNumber)}</td></tr><tr><td><strong>Totale listino al pubblico</strong></td><td>€ ${escapeHtml(payload.listSubtotalNet)} IVA inclusa</td></tr><tr><td><strong>Fascia sconto</strong></td><td>${escapeHtml(payload.reachedTierName)} · −${escapeHtml(discountPercent)}%</td></tr><tr><td><strong>Righe carrello</strong></td><td>${payload.itemCount}</td></tr></table><p style="margin:24px 0 0;color:#61705E;font-size:13px">ID ordine prospect: ${escapeHtml(payload.simulationId)}</p></main></body></html>`;
  return { subject, html };
}
...
export async function sendProspectSimulationNotification(
  ...
  try {
    ...
    const { subject, html } = buildProspectOrderNotification(payload);
